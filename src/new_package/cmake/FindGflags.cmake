...
# configuration (generated by gflags >= 2.1 which are built with CMake), or by
# performing a standard search for all gflags components.  The order of
...
#
# GFLAGS_FOUND: TRUE iff gflags is found. GFLAGS_INCLUDE_DIRS: Include
# directories for gflags. GFLAGS_LIBRARIES: Libraries required to link gflags.
# GFLAGS_NAMESPACE: The namespace in which gflags is defined.  In versions of
# gflags < 2.1, this was google, for versions >= 2.1 it is by default gflags,
# although can be configured when building gflags to be something else (i.e.
# google for legacy compatibility). FOUND_INSTALLED_GFLAGS_CMAKE_CONFIGURATION:
# True iff the version of gflags found was built & installed / exported as a
# CMake package.
#
...
# then prefer using an exported CMake configuration generated by gflags >= 2.1
# over searching for the gflags components manually.  Otherwise (FALSE) ignore
# any exported gflags CMake configurations and always perform a manual search
# for the components. Default: TRUE iff user does not define this variable
# before we are called, and does NOT specify either GFLAGS_INCLUDE_DIR_HINTS or
# GFLAGS_LIBRARY_DIR_HINTS otherwise FALSE. GFLAGS_INCLUDE_DIR_HINTS: List of
# additional directories in which to search for gflags includes, e.g:
# /timbuktu/include. GFLAGS_LIBRARY_DIR_HINTS: List of additional directories in
# which to search for gflags libraries, e.g: /timbuktu/lib.
#
...
#
# GFLAGS_INCLUDE_DIR: Include directory for gflags, not including the include
# directory of any dependencies. GFLAGS_LIBRARY: gflags library, not including
# the libraries of any dependencies.
...
macro(GFLAGS_REPORT_NOT_FOUND REASON_MSG)
  ...
  # Do not use unset, as we want to keep GFLAGS_NAMESPACE in the cache, but
  # simply clear its value.
  set(GFLAGS_NAMESPACE
      ""
      CACHE STRING "gflags namespace (google or gflags)" FORCE)

  # Make results of search visible in the CMake GUI if gflags has not been found
  # so that user does not have to toggle to advanced view.
  mark_as_advanced(CLEAR GFLAGS_INCLUDE_DIR GFLAGS_LIBRARY GFLAGS_NAMESPACE)
...
# which is blank (empty string, will test FALSE is CMake conditionals) if
# detection failed.
function(GFLAGS_CHECK_GFLAGS_NAMESPACE_USING_TRY_COMPILE)
  # Verify that all required variables are defined.
  gflags_check_vars_defined(GFLAGS_INCLUDE_DIR GFLAGS_LIBRARY)
  # Ensure that GFLAGS_NAMESPACE is always unset on completion unless we
  # explicitly set if after having the correct namespace.
  set(GFLAGS_NAMESPACE
      ""
      PARENT_SCOPE)
  ...
  # First try the (older) google namespace.  Note that the output variable MUST
  # be unique to the build type as otherwise the test is not repeated as it is
  ...
  # Try (newer) gflags namespace instead.  Note that the output variable MUST be
  # unique to the build type as otherwise the test is not repeated as it is
  # assumed to have already been performed.
  set(CMAKE_REQUIRED_INCLUDES ${GFLAGS_INCLUDE_DIR})
  set(CMAKE_REQUIRED_LIBRARIES ${GFLAGS_LIBRARY} ${GFLAGS_LINK_LIBRARIES})
  check_cxx_source_compiles(
...
# gflags_declare.h, which contains the namespace on versions >= 2.1.2. In
# general, this method should only be used when
...
function(GFLAGS_CHECK_GFLAGS_NAMESPACE_USING_REGEX)
  ...
  # Ensure that GFLAGS_NAMESPACE is always undefined on completion unless we
  # explicitly set if after having the correct namespace.
  set(GFLAGS_NAMESPACE
      ""
      PARENT_SCOPE)

  # Scan gflags.h to identify what namespace gflags was built with.  On versions
  # of gflags < 2.1.2, gflags.h was configured with the namespace directly, on
  # >= 2.1.2, gflags.h uses the GFLAGS_NAMESPACE #define which is defined in
  # gflags_declare.h, we try each location in turn.
  ...
  string(REGEX MATCH "namespace [A-Za-z]+" GFLAGS_NAMESPACE
               "${GFLAGS_HEADER_FILE_CONTENTS}")
  string(REGEX REPLACE "namespace ([A-Za-z]+)" "\\1" GFLAGS_NAMESPACE
                       "${GFLAGS_NAMESPACE}")
  ...
  if(GFLAGS_NAMESPACE STREQUAL "google" OR GFLAGS_NAMESPACE STREQUAL "gflags")
  ...
  string(REGEX MATCH "#define GFLAGS_NAMESPACE [A-Za-z]+" GFLAGS_NAMESPACE
...
if(NOT DEFINED GFLAGS_PREFER_EXPORTED_GFLAGS_CMAKE_CONFIGURATION
  ...
  message(
    STATUS
      "No preference for use of exported gflags CMake configuration "
...
if(GFLAGS_PREFER_EXPORTED_GFLAGS_CMAKE_CONFIGURATION)
  ...
  find_package(gflags QUIET NO_MODULE NO_CMAKE_PACKAGE_REGISTRY
  ...
  if(gflags_FOUND)
    ...
    message(
      STATUS "Failed to find installed gflags CMake configuration, "
             "searching for gflags build directories exported with CMake.")
    # Again pass NO_CMAKE_BUILDS_PATH, as we know that gflags is exported and do
    # not want to treat projects built with the CMake GUI preferentially.
    find_package(gflags QUIET NO_MODULE NO_CMAKE_BUILDS_PATH)
    if(gflags_FOUND)
  ...
  # whereby gflags_LIBRARIES = "gflags", but there was no imported target called
  # "gflags", they were called: gflags[_nothreads]-[static/shared]. As this
  # causes linker errors when gflags is not installed in a location on the
  # current library paths, detect if this problem is present and fix it.
  #
  # [1] https://github.com/gflags/gflags/issues/110
  if(gflags_FOUND)
    # NOTE: This is not written as additional conditions in the outer if
    # (gflags_FOUND) as the NOT TARGET "${gflags_LIBRARIES}" condition causes
    # problems if gflags is not found.
    if(${gflags_VERSION} VERSION_LESS 2.1.3 AND NOT TARGET
                                                "${gflags_LIBRARIES}")
      message(
        STATUS
          "Detected broken gflags install in: ${gflags_DIR}, "
      ...
      list(APPEND CHECK_GFLAGS_IMPORTED_TARGET_NAMES gflags-shared
           gflags-static gflags_nothreads-shared gflags_nothreads-static)
      ...
      if(NOT TARGET ${gflags_LIBRARIES})
        message(
          STATUS
            "Failed to fix detected broken gflags install in: "
  ...
  if(FOUND_INSTALLED_GFLAGS_CMAKE_CONFIGURATION)
    ...
    # gflags does not export the namespace in their CMake configuration, so use
    # our function to determine what it should be, as it can be either gflags or
    # google dependent upon version & configuration.
    #
    # NOTE: We use the regex method to determine the namespace here, as
    # check_cxx_source_compiles() will not use imported targets, which is what
    # gflags will be in this case.
    ...
    message(
      STATUS "Failed to find an installed/exported CMake configuration "
...
if(NOT GFLAGS_FOUND)
  # Either failed to find an exported gflags CMake configuration, or user told
  # us not to use one.  Perform a manual search for all gflags components.
  ...
  list(
    ...
    /usr/include)
  list(
    APPEND GFLAGS_CHECK_PATH_SUFFIXES gflags/include # Windows (for C:/Program
                                                     # Files prefix).
    gflags/Include) # Windows (for C:/Program Files prefix).

  list(APPEND GFLAGS_CHECK_LIBRARY_DIRS /usr/local/lib
       /usr/local/homebrew/lib # Mac OS X.
       /opt/local/lib /usr/lib)
  list(APPEND GFLAGS_CHECK_LIBRARY_SUFFIXES gflags/lib # Windows (for C:/Program
                                                       # Files prefix).
  ...
  endif(NOT GFLAGS_INCLUDE_DIR OR NOT EXISTS ${GFLAGS_INCLUDE_DIR})
  ...
  if(NOT GFLAGS_LIBRARY OR NOT EXISTS ${GFLAGS_LIBRARY})
    gflags_report_not_found("Could not find gflags library, set GFLAGS_LIBRARY "
                            "to full path to libgflags.")
  endif(NOT GFLAGS_LIBRARY OR NOT EXISTS ${GFLAGS_LIBRARY})
  ...
  # Mark internally as found, then verify. GFLAGS_REPORT_NOT_FOUND() unsets if
  # called.
  ...
  if(GFLAGS_INCLUDE_DIR AND NOT GFLAGS_NAMESPACE)
    ...
    # determine the gflags namespace.  Whilst this is less robust than 1), it
    # does avoid any interaction with msbuild.
    ...
    if(NOT GFLAGS_NAMESPACE)
      ...
      message(
        STATUS "Failed to find gflags namespace using using "
  ...
  # Make the GFLAGS_NAMESPACE a cache variable s/t the user can view it, and
  # could overwrite it in the CMake GUI.
  set(GFLAGS_NAMESPACE
      "${GFLAGS_NAMESPACE}"
      CACHE STRING "gflags namespace (google or gflags)" FORCE)

  # gflags does not seem to provide any record of the version in its source
  # tree, thus cannot extract version.

  # Catch case when caller has set GFLAGS_NAMESPACE in the cache / GUI with an
  # invalid value.
  if(GFLAGS_NAMESPACE
     AND NOT GFLAGS_NAMESPACE STREQUAL "google"
     AND NOT GFLAGS_NAMESPACE STREQUAL "gflags")
    gflags_report_not_found(
  ...
  if(GFLAGS_INCLUDE_DIR AND NOT EXISTS ${GFLAGS_INCLUDE_DIR}/gflags/gflags.h)
    gflags_report_not_found(
      "Caller defined GFLAGS_INCLUDE_DIR:"
      " ${GFLAGS_INCLUDE_DIR} does not contain gflags/gflags.h header.")
  endif(GFLAGS_INCLUDE_DIR AND NOT EXISTS ${GFLAGS_INCLUDE_DIR}/gflags/gflags.h)
  # TODO: This regex for gflags library is pretty primitive, we use lowercase
  # for comparison to handle Windows using CamelCase library names, could this
  # check be better?
  string(TOLOWER "${GFLAGS_LIBRARY}" LOWERCASE_GFLAGS_LIBRARY)
  if(GFLAGS_LIBRARY AND NOT "${LOWERCASE_GFLAGS_LIBRARY}" MATCHES
                        ".*gflags[^/]*")
    gflags_report_not_found("Caller defined GFLAGS_LIBRARY: "
                            "${GFLAGS_LIBRARY} does not match gflags.")
  endif(GFLAGS_LIBRARY AND NOT "${LOWERCASE_GFLAGS_LIBRARY}" MATCHES
                           ".*gflags[^/]*")
...
find_package_handle_standard_args(Gflags DEFAULT_MSG GFLAGS_INCLUDE_DIRS
                                  GFLAGS_LIBRARIES GFLAGS_NAMESPACE)

# Only mark internal variables as advanced if we found gflags, otherwise leave
